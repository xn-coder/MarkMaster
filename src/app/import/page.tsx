...
import { importDataAction } from '@/app/admin/actions';
...
export default function ImportDataPage() {
  ...
  // --- CORRECTED: handleDownloadSampleFile - workbook initialized before use ---
  const handleDownloadSampleFile = () => {
    ...
    // Student Marks Details Sheet - ADDING NEW PASS MARKS COLUMNS
    const studentMarksHeaders = ["Student ID", "Name", "Subject Name", "Subject Category", "Max Marks", "Theory Pass Marks", "Practical Pass Marks", "Theory Marks Obtained", "Practical Marks Obtained"];
    const sampleMarkRow = ["S001", "John Doe", "Physics", "Elective", 100, 21, 9, 65, 25];
    ...
    // Initialize workbook BEFORE appending sheets
  ...
  const handleImport = async () => {
    if (!file) { toast({ title: "No File Selected", description: "Please select an Excel file to import.", variant: "destructive" }); return; }
    if (!selectedAcademicSession || selectedAcademicSession === 'Select Session') { toast({ title: "No Session Selected", description: "Please select an Academic Session from the dropdown.", variant: "destructive" }); return; }
    const academicYearParts = selectedAcademicSession.split('-');
    if (academicYearParts.length !== 2 || !/^\d{4}$/.test(academicYearParts[0].trim()) || !/^\d{4}$/.test(academicYearParts[1].trim())) { toast({ title: "Invalid Session Format", description: `The selected Academic Session "${selectedAcademicSession}" is not valid. Expected YYYY-YYYY.`, variant: "destructive" }); return; }
    ...
    reader.onload = async (e) => {
      ...
      if (!dataUrl) { toast({ title: "File Read Error", description: "Could not read the file content for upload.", variant: "destructive" }); setIsLoading(false); return; }
      ...
      try {
        ...
        toast({ title: overallSuccess ? "Import Processed" : "Import Processed with Issues", description: primaryMessage?.message || "Review the details below.", variant: overallSuccess ? "default" : (resultsFromServer.summaryMessages.some(m => m.type === 'error') ? "destructive" : "default"), });
      } catch (error: any) { console.error("Error calling importDataAction:", error); toast({ title: "Import Failed", description: error.message || "An unknown error occurred during server processing.", variant: "destructive" }); setImportResults({ summaryMessages: [{ type: 'error', message: `Client-side error calling server action: ${error.message}` }], studentFeedback: [], marksFeedback: [], totalStudentsProcessed: 0, totalStudentsAdded: 0, totalStudentsSkipped: 0, totalMarksProcessed: 0, totalMarksAdded: 0, totalMarksSkipped: 0, }); } finally { setIsLoading(false); if (fileInputRef.current) { fileInputRef.current.value = ''; } setFile(null); }
    };
    reader.onerror = () => { toast({ title: "File Read Error", description: "Could not read the file to prepare for upload.", variant: "destructive" }); setIsLoading(false); };
  };
          ...
          <Card> 
            <CardHeader>
              ...
              <CardDescription className="space-y-1">
                ...
                <p className="text-sm font-medium">
                  Required columns for "Student Marks Details" sheet: Student ID, Name, Subject Name, Subject Category, Max Marks, <strong>Theory Pass Marks, Practical Pass Marks,</strong> Theory Marks Obtained, Practical Marks Obtained.
                </p>
              </CardDescription>
