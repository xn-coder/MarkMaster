...
export function MarksheetDisplay({ data, onCreateNew, onEditBack }: MarksheetDisplayProps) {
  ...
  return (
    <div className="marksheet-print-container bg-gray-100 print:bg-white py-8 print:py-0 flex justify-center items-center flex-col">
       <div className="flex flex-col sm:flex-row justify-center items-center gap-3 p-4 print:hidden mb-6">
        {onEditBack && (
      ...
      <Card
        ref={marksheetRef}
        className="marksheet-preview marksheet-card w-full max-w-[210mm] min-h-[297mm] shadow-lg print:shadow-none border-4 border-black print:border-4 print:border-black print:my-0 print:mx-0 bg-white p-1 print:p-0.5 relative"
      >
        ...
        <div className="border border-black flex-grow print:border-black p-4 print:p-3 flex flex-col relative z-10 print:border print:border-black h-full">
          <header className="relative mb-2 print:mb-1.5">
            ...
            <div className="text-center mt-1 print:mt-0.5 mb-3 print:mb-2">
              <div className="rounded-md inline-block mx-auto" style={{ backgroundColor: '#DB2A2A' }}>
                <h2 className="text-white text-lg print:text-base font-bold uppercase px-4 py-1">MARKSHEET</h2>
              </div>
            </div>
          </header>

          <h3 className="text-center font-bold text-xl print:text-lg underline decoration-1 underline-offset-2 mb-3 print:mb-2 text-black">Student Details</h3>

          <div className="mb-3 print:mb-2 grid grid-cols-2 gap-x-12 print:gap-x-10 gap-y-1.5 print:gap-y-1 text-base print:text-sm font-sans text-black">
            <div>Student Name: <strong className="font-semibold">{data.studentName}</strong></div>
            <div className="text-right">Registration No: <strong className="font-semibold">{data.registrationNo}</strong></div>
            ...
            <div>Date of Birth: <strong className="font-semibold">{toDate(data.dateOfBirth).toLocaleDateString('en-GB')}</strong></div>
            <div className="text-right">Faculty: <strong className="font-semibold">{data.faculty}</strong></div>
          ...
          <div className="flex-grow flex flex-col">
            <section className="my-3 print:my-1.5">
              <div className="overflow-x-hidden print:overflow-x-hidden">
                <Table className="border-collapse text-black print:text-black text-xs print:text-[9px] bg-transparent print:bg-transparent">
                  <TableHeader className="bg-gray-100 print:bg-gray-100 hover:bg-gray-100 print:hover:bg-gray-100">
                    <TableRow className="border-b border-black bg-gray-100 print:bg-gray-100 hover:bg-gray-100 print:hover:bg-gray-100 dark:hover:bg-inherit print:h-[20px]">
                      <TableHead rowSpan={2} className="w-[8%] text-center border border-black font-bold text-black align-middle p-1 print:p-0.5 text-sm print:text-xs">Sr. no.</TableHead>
                      ...
                      <TableHead rowSpan={2} className="w-[12%] text-center border border-black font-bold text-black align-middle p-1 print:p-0.5 text-sm print:text-xs">Passing Marks</TableHead>
                    ...
                    <TableRow className="border-b border-black bg-gray-100 print:bg-gray-100 hover:bg-gray-100 print:hover:bg-gray-100 dark:hover:bg-inherit font-bold text-black text-sm print:text-xs">
                  ...
                  <TableBody className="text-xs print:text-[9px]">
                    ...
                    {compulsorySubjects.map((subject, index) => (
                      <TableRow key={`comp-${index}`} className="border-b border-black h-[22px] print:h-[18px] hover:bg-transparent print:hover:bg-transparent dark:hover:bg-inherit">
                        <TableCell className="text-center border border-black p-1 print:p-0.25">{index + 1}</TableCell>
                        <TableCell className="border border-black p-1 print:p-0.25 text-left bg-transparent print:bg-transparent">{subject.subjectName}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.totalMarks}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.passMarks}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.theoryMarksObtained}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.practicalMarksObtained}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25 font-bold text-black">{subject.obtainedTotal}</TableCell>
                      </TableRow>
                    ))}
                    ...
                    {electiveSubjects.map((subject, index) => (
                      <TableRow key={`elec-${index}`} className="border-b border-black h-[22px] print:h-[18px] hover:bg-transparent print:hover:bg-transparent dark:hover:bg-inherit">
                        <TableCell className="text-center border border-black p-1 print:p-0.25">{compulsorySubjects.length + index + 1}</TableCell>
                        <TableCell className="border border-black p-1 print:p-0.25 text-left">{subject.subjectName}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.totalMarks}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.passMarks}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.theoryMarksObtained}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.practicalMarksObtained}</TableCell>
                    ...
                    {additionalSubjects.map((subject, index) => (
                      <TableRow key={`addl-${index}`} className="border-b border-black h-[22px] print:h-[18px] hover:bg-transparent print:hover:bg-transparent dark:hover:bg-inherit">
                        <TableCell className="text-center border border-black p-1 print:p-0.25">{compulsorySubjects.length + electiveSubjects.length + index + 1}</TableCell>
                        <TableCell className="border border-black p-1 print:p-0.25 text-left bg-transparent print:bg-transparent">{subject.subjectName}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.totalMarks}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.passMarks}</TableCell>
                        <TableCell className="text-right border border-black p-1 print:p-0.25">{subject.theoryMarksObtained}</TableCell>
                    ...
                    <TableRow className="border-t-2 border-black h-[24px] print:h-[22px] hover:bg-transparent print:hover:bg-transparent dark:hover:bg-inherit">
                      ...
                      <TableCell className="text-right border-r border-b border-black p-1 print:p-0.5 font-bold text-base print:text-sm text-black">{data.aggregateMarksCompulsoryElective}</TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              </div>
            </section>
            <div className="mt-3 print:mt-1.5 mb-2 print:mb-1 text-base print:text-sm text-black">
          ...
          <div className="mt-auto pt-10 print:pt-8 text-sm print:text-xs text-black">
      ...
      <style jsx global>{`
        @media print {
          body, html {
            ...
            font-family: 'Times New Roman', Times, serif; /* Default print font */
          }
          ...
          .marksheet-card {
            ...
            padding: 2pt !important; /* Space for outer border */
            margin: 0 !important;
            background-color: white !important;
          }
          .marksheet-card > div:first-child { /* Inner content div */
            padding: 0.8cm !important; /* Inner padding for content */
            border-width: 1px !important;
            border-color: black !important;
            margin: 0 !important;
            height: 100% !important; /* Allow this to stretch */
            display: flex !important;
            flex-direction: column !important;
            box-sizing: border-box !important;
          }
          ...
          .print\\:text-\\[11px\\] { font-size: 11pt !important; line-height: 1.2 !important;}
          ...
          .print\\:text-xs { font-size: 10pt !important; line-height: 1.2 !important; } /* Increased from 8pt */
          .print\\:text-sm { font-size: 11pt !important; line-height: 1.2 !important; } /* Increased from 9pt */
          .print\\:text-base { font-size: 12pt !important; line-height: 1.2 !important; } /* Increased from 10pt */
          .print\\:text-lg { font-size: 14pt !important; line-height: 1.2 !important; } /* Increased from 11pt */
          .print\\:text-xl { font-size: 16pt !important; line-height: 1.2 !important; } /* Increased from 12pt */
          .print\\:text-2xl { font-size: 18pt !important; line-height: 1.2 !important; } /* Increased from 14pt */
          ...
          .print\\:pt-\\[0\\.5cm\\] { padding-top: 0.5cm !important;} /* Example for specific cm padding */
          .print\\:pt-8 { padding-top: 2cm !important; } /* Increased from pt-10 */
          .print\\:p-0\\.5 {padding: 2pt !important;}
          .print\\:p-1 {padding: 4pt !important;}
          .print\\:p-0\\.25 {padding: 1pt !important;}
          .print\\:p-2\\.5 {padding: 0.25cm !important;} /* Adjusted for consistency */
          ...
          @page {
            size: A4 portrait;
             margin: 1.5cm 1.2cm 1.5cm 1.2cm; /* Previous standard margins */
           /* margin: 0.5cm; Ensures 0.5cm padding from document edge to content */
          }
        }
      `}</style>
    </div>
...
now in marksheet, instead of marksheet no. keep registration no. by user fills or importing data